...
import { cn } from "../../../shared/utils/arrayUtils";
...
export const Turnstile = ({
  ...
  useEffect(() => {
    ...
    const renderWidget = async () => {
      try {
        ...
        setReady(false);
        onError?.(
          err instanceof Error ? err.message : "Turnstile failed to load",
        );
      }
    };
  ...
  return (
    <div className={cn("flex flex-col items-start gap-1", className)}>
      ...
      {!ready && (
        <span className="text-xs text-gray-500">Loading verification…</span>
      )}
    </div>
  );
};
