...
export const TopAppBarSection = ({
  onShowDownloadPopup,
  initialSearchText,
}: TopAppBarSectionProps): React.ReactElement => {
  ...
  const renderNav = (className = "") => (
    <nav className={`flex items-center gap-3 ${className}`}>
      ...
      <button
        ...
        onClick={onShowDownloadPopup}
      >
        <img className="w-[34px] h-[30px]" alt="Shopping cart" src={iconCart} />
      ...
      <button
        ...
        onClick={onShowDownloadPopup}
      >
        <img className="w-[33px] h-[31px]" alt="Profile" src={iconProfile} />
        <span className="[font-family:'Nunito',Helvetica] font-normal text-black text-xs text-center tracking-[0.40px] leading-4 whitespace-nowrap">
          Profile
        </span>
      </button>
      ...
      <span className="mx-2" />
      <Button
        className="h-[45px] w-[110px] bg-[#fe2188] hover:bg-[#fe2188]/90 rounded-[10px] [font-family:'Montserrat',Helvetica] font-bold text-white text-[18px] tracking-[0] leading-[normal]"
        onClick={onShowDownloadPopup}
      >
        SELL
      </Button>
    ...
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200">
      <div className="max-w-md mx-auto px-4 py-2 flex items-center justify-between">
        ...
        >
          ...
          >
            <path
              d="M4 11.5V20c0 .55.45 1 1 1h14c.55 0 1-.45 1-1v-8.5c0-.27-.11-.52-.29-.71l-7-7a1 1 0 0 0-1.42 0l-7 7A1 1 0 0 0 4 11.5Z"
              fill="#fe2188"
            />
  ...
  return (
    <header className="w-full bg-white pb-2 sm:pb-6 lg:pb-5 pt-1 px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="hidden lg:flex items-center justify-between gap-8 px-4">
        ...
        <div className="flex-1 max-w-[532px]">{renderSearchBar()}</div>
        {renderNav()}
      </div>
